  ...
  if(values === 1) {
    ...
    if(!channel) return;
        ...
        switch(change) {
            ...
            .setDescription(`🎉 **| ${newMember.user} given a role \`${addedRole}\`**`)
            ...
            .setDescription(`😢 **| ${newMember} **role \`${removedRole}\` has been remove for you**`)
            .setFooter(`ID: ${newMember.user.id}`).setTimestamp() 
                channel.send(embed3)
                break;
          ...
          .setDescription(`🖼** | ${newMember.user.tag} has changed the avatar**`)
          .setFooter(`ID: ${newMember.user.id}`).setTimestamp() 
          channel.send(embed6)
                break;
    }
